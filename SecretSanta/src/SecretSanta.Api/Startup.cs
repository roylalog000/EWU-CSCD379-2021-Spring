...
namespace SecretSanta.Api
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddDbContext<Data.DbContext>(options => options.UseSqlite(Configuration.GetConnectionString("DefaultConnection")));

            services.AddDbContext<SecretSanta.Data.DbContext>(options => 
            {
            options.UseSqlite(Configuration.GetConnectionString("DefaultConnection"));
            options.UseLoggerFactory(LoggerFactory.Create( builder => 
                        builder.AddSerilog(Log.Logger.ForContext<Data.DbContext>().ForContext("Category", "Database"))
            ));
